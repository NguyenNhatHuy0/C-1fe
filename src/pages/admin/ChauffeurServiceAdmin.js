...
const ChauffeurServiceAdmin = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await fetch(`http://localhost:8080/findAllChauffeurService`, {
                method: 'GET',
                headers: {
    ...
    const saveHandle = async (idCar) => {
        try {
            const response = await fetch(`http://localhost:8080/updateStatusDriver/${idCar}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                                    ...
                                    <td class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b block lg:table-cell relative lg:static">
                                        {
                                            (item.status == "YES") ? (
                                                ...
                                                <>
                                                    <button onClick={() => saveHandle(item.id)} class="rounded bg-green-600 py-1 px-3 text-xs font-bold hover:bg-blue-800 mr-[5px]">Xử lý</button>
...
export default ChauffeurServiceAdmin